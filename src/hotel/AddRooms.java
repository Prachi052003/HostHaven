...
public class AddRooms extends JFrame {
	...
	public void addRooms() {
		...
		try {
			...
			if(ps.executeUpdate()>0) {
				...
				Object[] newRow = {r1.getText(), r2.getText(), r3.getText()};
				model.addRow(newRow);
			}
		}
		catch(Exception ex) {
	...
	public void updateRooms()
	{
		...
		try {
			...
			if(r1.getText().length() > 0) ps.setString(1,r1.getText());
			else ps.setString(1, model.getValueAt(i, 1).toString());
			if(r2.getText().length() > 0) ps.setString(2,r2.getText());
			else ps.setString(2, model.getValueAt(i, 2).toString());
			if(r3.getText().length() > 0) ps.setString(3,r3.getText());
			else ps.setString(3, model.getValueAt(i, 3).toString());
			ps.setString(4, no);
			
			if(ps.executeUpdate()>0) {
				JOptionPane.showMessageDialog(null, " Room Updated ");
				if(r1.getText().length() > 0) model.setValueAt(r1.getText(), i, 1);
				if(r2.getText().length() > 0) model.setValueAt(r2.getText(), i, 2);
				if(r3.getText().length() > 0) model.setValueAt(r3.getText(), i, 3);
			}	
		}
		catch(Exception ex) {
			ex.printStackTrace();
